...
생성일 2026-09-23 · 모듈 31개 · 합계 15,951줄
...
| [`fluids.js`](../src/fluids.js) | 물 흐름 · 낙하 블록 · 잎 부패 | 970 | state · atlas · settings · queues · dims · blocks · world · tree · light · mesh · scene · audio · player · survival · edit |
...
| [`edit.js`](../src/edit.js) | 편집 · 되돌리기 · 도전 과제 | 1562 | state · queues · settings · save · dims · blocks · world · light · fluids · mesh · player · audio · hud · hand · survival · sky |
...
| [`input.js`](../src/input.js) | 입력 (키보드 · 마우스 · 터치) | 2223 | state · world · queues · mobs · dims · mesh · light · boot · blocks · survival · village · scene · daynight · settings · player · audio · save · cloud · edit · hud · hand · mine · sky · loop |
| [`mine.js`](../src/mine.js) | 캐기 · 놓기 | 491 | state · mobs · fluids · dims · blocks · world · light · scene · player · audio · edit · hud · survival · hand · input |
...
| [`loop.js`](../src/loop.js) | 게임 루프 | 1117 | state · input · mobs · queues · dims · blocks · atlas · world · light · village · fluids · mesh · scene · daynight · settings · player · audio · save · edit · hud · atlas · survival · hand · body · mine · sky |
...
| [`survival.js`](../src/survival.js) | 모으기 모드: 가방 · 캐면 얻기 · 곡괭이 단계 · 제작대/화로 제작 · 목표 한 줄 | 333 | state · blocks · world · player · dims · hud · input |
...
| `primeTick(dt)` | 795 |
| `explode(cx, cy, cz, radius, selfTnt)` | 811 |
| `enqueueGrow(x, y, z)` | 853 |
| `growTick(dt)` | 883 |
...
| `beginBatch(cap)` | 189 |
| `ownFire()` | 198 |
| `settleWorld(list)` | 210 |
| `notePlaced(b, sh, n)` | 249 |
| `liftIfBuried()` | 268 |
| `endBatch(label, credit)` | 290 |
| `editLabel(e)` | 367 |
| `svBlocked()` | 389 |
| `undo()` | 394 |
| `redo()` | 418 |
...
| `nextToTry(n)` | 515 |
| `refreshAchList()` | 530 |
| `checkFoundAchievements()` | 557 |
| `checkBuildAchievements()` | 583 |
| `refreshStats()` | 714 |
| `achCount()` | 732 |
| `unlock(id)` | 737 |
| `selectionBounds()` | 764 |
| `selectionSize()` | 765 |
| `fillSelection(block, sh, only)` | 775 |
| `clearSelection()` | 795 |
| `shellSelection(block, sh, mode)` | 814 |
| `roundSelection(block, sh, r, h, hollow, kind)` | 845 |
| `copySelection()` | 878 |
| `mirrorClip()` | 920 |
| `rotateClip()` | 939 |
| `pasteClip(px, py, pz, withAir)` | 963 |
| `completeCommand(prefix)` | 1045 |
| `runCommand(line)` | 1052 |
| `loadBlueprints()` | 1431 |
| `saveBlueprint(name)` | 1434 |
| `useBlueprint(name)` | 1460 |
| `exportBlueprint(name)` | 1482 |
| `importBlueprint(text)` | 1497 |
| `blueprintNames()` | 1522 |
| `blueprintList()` | 1525 |
| `deleteBlueprint(name)` | 1535 |
| `selectionCounts()` | 1545 |
...
| `cycleMinimapZoom(dir)` | 1210 |
| `swapBarPage()` | 1225 |
| `pickBlock()` | 1249 |
| `setStick(dx, dy)` | 1645 |
| `bindHold(id, onDown, onUp)` | 1789 |
| `toggleRegionBar(on)` | 1879 |
| `bindOpt(inputId, outId, key, fmt)` | 2045 |
| `refreshScaleLabels()` | 2075 |
| `pollGamepadMenu()` | 2120 |
| `pollGamepad(dt)` | 2130 |
...
| `tryInteract(hit)` | 248 |
| `doorOther(x, y, z)` | 280 |
| `scoopLiquid(repeating)` | 312 |
| `pourLiquid(hit, repeating)` | 335 |
| `place(repeating)` | 355 |
...
| `seedMobs()` | 163 |
| `seedVillage(v)` | 193 |
| `anyMobNear(px, pz, r)` | 316 |
| `updateMobs(dt)` | 349 |
| `pushOutOfMobs(px, pz, half)` | 518 |
| `mobOccupies(x, y, z)` | 538 |
| `aimedMob(maxDist)` | 554 |
| `aimingAtMob()` | 574 |
| `removeMob(m)` | 579 |
| `feedNearbyMob(pos, prefer)` | 597 |
| `breedTick(dt)` | 652 |
| `setMobsVisible(on)` | 690 |
| `seedFlocks()` | 711 |
| `updateFlocks(dt)` | 728 |

내보내는 값 — `LOVE_HINT` · `MOB_COUNT` · `MOB_MAX` · `BREED_COOL` · `FISH_COUNT` · `BIRD_COUNT` · `MOB_KINDS` · `mobs` · `mobGroup` · `SHEAR_MS` · `HERD` · `fish` · `birds`
...
| `svEvent(key)` | 246 |
| `collect(b)` | 279 |
| `leafDrop(b, x, y, z)` | 291 |
| `enrichDiamonds(seed, rng)` | 304 |
| `resetSurvival(on)` | 321 |
