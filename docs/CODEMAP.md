...
생성일 2026-09-08 · 모듈 28개 · 합계 10,568줄
...
| 모듈 | 하는 일 | 줄 | 기대는 곳 |
|---|---|---:|---|
| [`state.js`](../src/state.js) | 여러 모듈이 값을 바꾸는 공유 상태 | 181 | — |
...
| [`blocks.js`](../src/blocks.js) | 블록 정의 · 모양 · 성질 | 289 | state |
| [`tree.js`](../src/tree.js) | 나무 한 그루의 모양 | 47 | — |
| [`atlas.js`](../src/atlas.js) | 텍스처 아틀라스 (코드로 그리는 16×16 도트) | 554 | blocks |
| [`world.js`](../src/world.js) | 월드 데이터 · 지형 생성 | 980 | state · tree · queues · dims · blocks · atlas |
...
| [`scene.js`](../src/scene.js) | three.js 씬 · 셰이더 · 파티클 | 571 | dims · boot · blocks · atlas · world · mesh |
| [`daynight.js`](../src/daynight.js) | 낮과 밤 | 83 | state · world · scene |
| [`settings.js`](../src/settings.js) | 설정 | 43 | state · boot · scene |
| [`player.js`](../src/player.js) | 플레이어 · 충돌 · 레이캐스트 | 323 | state · dims · blocks · world · scene |
...
| [`edit.js`](../src/edit.js) | 편집 · 되돌리기 · 도전 과제 | 991 | state · queues · settings · save · dims · blocks · world · light · fluids · mesh · player · audio · hud · sky |
| [`hud.js`](../src/hud.js) | HUD · 핫바 · 블록 고르기 · 미니맵 | 514 | state · version · dims · blocks · atlas · world · player · hand · input |
| [`hand.js`](../src/hand.js) | 1인칭 손과 들고 있는 블록 | 165 | state · settings · blocks · atlas · world · mesh · scene · player · dims · light · daynight |
| [`input.js`](../src/input.js) | 입력 (키보드 · 마우스 · 터치) | 1456 | state · world · queues · mobs · dims · mesh · light · boot · blocks · scene · daynight · settings · player · audio · save · cloud · edit · hud · hand · mine · sky · loop |
| [`mine.js`](../src/mine.js) | 캐기 · 놓기 | 219 | state · mobs · fluids · dims · blocks · world · scene · player · audio · edit · hud · hand · input |
| [`mobs.js`](../src/mobs.js) | 걸어 다니는 동물. 세계에 "살아 있는 것" 을 하나 넣는다. | 537 | dims · world · blocks · scene · player · audio |
...
| [`loop.js`](../src/loop.js) | 게임 루프 | 863 | state · input · mobs · queues · dims · blocks · atlas · world · light · fluids · mesh · scene · daynight · settings · player · audio · save · edit · hud · hand · mine · sky |
| [`version.js`](../src/version.js) | 빌드 도장 (자동 생성) | 8 | — |
| [`main.js`](../src/main.js) | 조립과 시작 | 253 | state · tree · mobs · atlas · queues · dims · blocks · world · light · fluids · mesh · scene · daynight · settings · player · audio · save · cloud · edit · hud · hand · input · mine · sky · loop |
...
| `markName(m)` | 212 |
| `underBand(y)` | 224 |
| `expandLegacySeen()` | 232 |
| `markSeen(px, pz, r, bit)` | 237 |
| `seenRatio()` | 252 |
| `markTouched(x, y, z)` | 258 |
| `isTouched(x, y, z)` | 261 |
| `refreshAllTops()` | 265 |
| `hash2(x, y, seed)` | 269 |
| `hash3(x, y, z, seed)` | 274 |
| `smooth(t)` | 280 |
| `lerp(a, b, t)` | 281 |
| `noise2(x, y, seed)` | 283 |
| `noise3(x, y, z, seed)` | 290 |
| `oreCeil()` | 301 |
| `lavaTop()` | 307 |
| `generate(seed, gen)` | 309 |

내보내는 값 — `world` · `shape` · `heightMap` · `topMap` · `biomeMap` · `waterLvl` · `BIOME_NAMES` · `touched` · `SEEN_TOP` · `UNDER_BANDS` · `SEEN_UNDER_ALL` · `seenMap` · `MOUTH_DEPTH` · `MINE_W`
...
| `outerSeaY()` | 146 |
| `updateOuterSea(camY)` | 212 |
| `updateChunkVisibility(farDist, floor, aboveGround)` | 249 |
| `boxesToEdges(boxes)` | 312 |
| `dynamicHighlight(boxes)` | 334 |
| `burst(x, y, z, blockId, count)` | 441 |
| `updateParticles(dt)` | 462 |
| `updateEdge(px, pz)` | 519 |
| `updatePasteBox(c, p)` | 547 |
| `updateSelectionBox(b, anchor)` | 557 |

내보내는 값 — `matOpaque` · `scene` · `stage` · `VOX_VS` · `VOX_FS` · `voxUniforms` · `skyUniforms` · `sky` · `outerSea` · `FREE_DIST` · `chunkFreed` · `BURIED_KEEP` · `chunkBuried` · `cloudMat` · `cloudMatHigh` · `cloudGroup` · `cloudGroupHigh` · `HL_EDGES` · `HL_GEO` · `HL_CROSS` · `SHAPE_BOUNDS` · `PRIMED_MAX` · `primedMat` · `primedBoxes` · `highlight` · `crackMat` · `crackMesh` · `PMAX` · `pPos` · `pVel` · `pCount` · `pGeo` · `pMat` · `particles` · `edgeMat` · `edgeGroup` · `SEL_DONE` · `selMat` · `selBox` · `pasteMat` · `pasteBox`
...
| `batchPush(b, x, y, z, from, to, fromSh, toSh, wl, toWl)` | 143 |
| `beginBatch(cap)` | 152 |
| `ownFire()` | 161 |
| `settleWorld(list)` | 173 |
| `endBatch(label)` | 208 |
| `editLabel(e)` | 258 |
| `undo()` | 278 |
| `redo()` | 297 |
| `refreshAchList()` | 357 |
| `checkBuildAchievements()` | 381 |
| `refreshStats()` | 488 |
| `achCount()` | 506 |
| `unlock(id)` | 511 |
| `selectionBounds()` | 538 |
| `selectionSize()` | 539 |
| `fillSelection(block, sh, only)` | 549 |
| `clearSelection()` | 568 |
| `copySelection()` | 582 |
| `mirrorClip()` | 624 |
| `rotateClip()` | 643 |
| `pasteClip(px, py, pz)` | 663 |
| `completeCommand(prefix)` | 714 |
| `runCommand(line)` | 721 |
| `loadBlueprints()` | 916 |
| `saveBlueprint(name)` | 919 |
| `useBlueprint(name)` | 935 |
| `blueprintNames()` | 951 |
| `blueprintList()` | 954 |
| `deleteBlueprint(name)` | 964 |
| `selectionCounts()` | 974 |

내보내는 값 — `HISTORY_MAX` · `FIRE_UNDO_MAX` · `FLUID_UNDO_MAX` · `BATCH_RELIGHT_ALL` · `HISTORY_CELLS_MAX` · `lastEditLabel` · `undoEmptyWhy` · `ACHIEVEMENTS` · `achGrid` · `BUILD_R` · `BUILD_IDS` · `statGrid` · `REGION_MAX` · `CMD_HELP` · `CMD_LIST` · `BP_KEY`
...
| `selectSlot(i)` | 82 |
| `openPicker()` | 128 |
| `closePicker(resume)` | 139 |
| `facingText()` | 152 |
| `showAchPop(name, desc)` | 163 |
| `showHud(on)` | 193 |
| `toast(msg)` | 199 |
| `refreshMinimapCap()` | 209 |
| `drawMinimap()` | 217 |
| `helpOpen()` | 368 |
| `toggleHelp(on)` | 369 |
| `setHelpTab(showAch)` | 390 |
| `bootProgress(msg, frac)` | 405 |
| `bootDone()` | 410 |
| `noteBlockUse(b)` | 421 |
| `sortPickByRecent()` | 427 |
| `refreshPickFilter()` | 437 |
| `openCmd()` | 464 |
| `closeCmd()` | 471 |
| `cmdSay(msg)` | 476 |
| `drawPreview(target)` | 482 |
...
| `pickBlock()` | 789 |
| `setStick(dx, dy)` | 1155 |
| `bindHold(id, onDown, onUp)` | 1257 |
| `bindOpt(inputId, outId, key, fmt)` | 1327 |
| `pollGamepadMenu()` | 1385 |
| `pollGamepad(dt)` | 1395 |
